...
class DevicesModel extends Model
{
    ...
    public function getDevicesInfo($map)
    {
        ...
        $count = $this
            ...
            ->field("statu.*,bind.*,d.id,d.device_code,type.*,vendors.*,d.name dname,d.phone,d.address")
            ->order('d.id asc')
        ...
        $data = $this
            ...
            ->join("__VENDORS__ vendors ON bind.vid=vendors.id", 'LEFT')
            ->join("__DEVICE_TYPE__ type ON d.type_id=type.id", 'LEFT')
            ->field("statu.*,bind.*,d.device_code,type.*,vendors.*,d.name dname,d.phone,d.address,d.id")
            ->order('d.id asc')
            ->limit($page->firstRow.','.$page->listRows)
            ->select();
        // 分配返回数据
